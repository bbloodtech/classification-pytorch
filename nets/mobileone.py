...
class MobileOneBlock(nn.Module):
    ...
    def __init__(self,
        ...
        if se_cfg is not None:
            self.se = SEModule(channels=out_channels, **se_cfg)
        else:
            self.se = nn.Identity()
        ...
        if deploy:
            ...
            self.branch_conv_list = nn.ModuleList()
...
def mobileone(pretrained=False, num_classes=1000):
    model = MobileOne('s0', out_indices=(3, ))
    if num_classes!=1000:
        model.classifier = nn.Sequential(
        nn.Dropout(0.2),
        nn.Linear(model.in_planes, num_classes),
        )
    return model
